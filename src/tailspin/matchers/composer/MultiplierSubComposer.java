...
public class MultiplierSubComposer implements SubComposer {
  ...
  private Memo addRepetitions(String s, Memo memo) {
    while (memo.pos < s.length() &&
        (null == multiplier.permeate(values.size(), null, scope, null)
            || null != multiplier.permeate(values.size()+1, null, scope, null))) {
  ...
  public boolean isSatisfied() {
    return values != null && null != multiplier.permeate(values.size(), null, scope, null);
  }
}
